...
export class SessionsService {
  ...
  async remove(id: number) {
    const existing = await this.findOne(id);
    const studentCount = await this.prisma.student.count({ where: { sessionId: id, isDeleted: false } });
    if (studentCount > 0) {
      throw new BadRequestException(`Cannot delete session with ${studentCount} enrolled student(s).`);
    }
